...
def split_train_test_clusters(measure, clu_thre, n_fold):
	...
	with open(cluster_path+measure+'_compound_cluster_dict_'+str(clu_thre), 'rb') as f:
		C_cluster_dict = pickle.load(f, encoding='latin1')
	with open(cluster_path+measure+'_protein_cluster_dict_'+str(clu_thre), 'rb') as f:
		P_cluster_dict = pickle.load(f, encoding='latin1')
	...
	pair_kf = KFold(n_fold,shuffle=True) # pair_kf = KFold(len(pair_list), n_fold, shuffle=True)
...
def load_data(measure, setting, clu_thre, n_fold):
	...
	cid_list = list(map(lambda x : x.decode(), data_pack[7]))
	pid_list = list(map(lambda x : x.decode(), data_pack[8]))
	...
	if setting == 'imputation':
		...
		for fold in range(n_fold):
			...
			for ele in range(n_sample):
				# print(ele)
				# print(list(cid_list)[1].decode())
				# print(list(C_cluster_dict.keys())[1])
